...
public class CmEthSerciePWInfo extends BaseVO {
	...
	String rmUID;
	/**
	 * 名称：承载伪线rmUID 最大长度: 是否必填：是 取值范围及说明： 数据示例：
	 * */
	String pwrmUID;
	/**
	 * 名称：承载伪线源端网元rmUID 最大长度: 是否必填：是 取值范围及说明：1、源端网元要求是以太网业务中的接入点网元之一 数据示例：
	 * */
	String aEndNermUID;
	/**
	 * 名称：承载伪线宿端网元rmUID 最大长度: 是否必填：是 取值范围及说明：1、宿端网元要求是以太网业务中的接入点网元之一 数据示例：
	 * */
	String zEndNermUID;
	/**
	 * 名称：承载伪线角色 最大长度: 是否必填：是 取值范围及说明：枚举值包括： Master：主用； Backup：备用； NA：无主备；
	 * IDI：华为在LTE业务中两个二转三节点网元间配置的伪线 数据示例：
	 * */
	String role;
