...
public class CmLabelSwitch extends BaseVO {
	...
	/**
	 * 名称：隧道rmUID 最大长度: 是否必填：是 取值范围及说明： 数据示例：
	 * */
	String tunnelrmUID;
	/**
	 * 名称：方向 最大长度: 是否必填：是 取值范围及说明：枚举值包括： CD_UNI：单向；CD_BI：双向 数据示例：CD_BI
	 * */
	String direction;
	/**
	 * 名称：归属路由组 最大长度: 是否必填：是 取值范围及说明：当隧道是双向而路由中的标签交换是单向时，需要分两组分别描述隧道经过的所有标签交换信息
	 * 取值只能为1或2。 1、当隧道和路由中的各标签交换都是双向或单向时，此项固定填数字1
	...
	String routingGroup;
	/**
	 * 名称：路由组中的序号 最大长度: 是否必填：是 取值范围及说明从1开始，十进制 数据示例：2
	 * */
	String routingNo;
	/**
	 * 名称：源端端口rmUID 最大长度: 是否必填：否 取值范围及说明1、对于路由的第1个节点，此项不填 2、当源端是MPLS保护环时，此项不填
	 * 3、所有其他节点，此项必填 数据示例：
	 * */
	String aEndPortrmUID = "";
	/**
	 * 名称：源端rmUID 最大长度: 是否必填：否 取值范围及说明：源端的唯一标识，可能是网元TP点rmUID也可能是MPLS环rmUID
	 * 1、当源端是网元中的TP点，此项填写TP点rmUID；当源端是MPLS环时，此项填写MPLS环rmUID
	 * 2、对于路由的第1个节点，此项不填；所有其他节点，此项必填 数据示例：
	 * */
	String aEndTprmUID = "";
	/**
	 * 名称：源端出标签 最大长度:20 是否必填：否 取值范围及说明：1、对于路由的第1个节点，此项不填 2、所有其他网元节点：
	 * （1）标签交换方向是双向时，此项必填 （2）标签交换方向是单向时，此项不填 3、当源端是MPLS保护环时，此项不填 数据示例：148
	 * */
	String aEndOutLabel = "";
	/**
	 * 名称：源端入标签 最大长度:20 是否必填：否 取值范围及说明：1、对于路由的第1个节点，此项不填 2、所有其他网元节点，此项必填
	 * 3、当源端是MPLS保护环时，此项不填
	 * 
	 * 数据示例：17
	 * */
	...
	/**
	 * 名称：宿端端口rmUID 最大长度: 是否必填：否 取值范围及说明1、对于路由的最后1个节点，此项不填 2、当宿端是MPLS保护环时，此项不填
	 * 3、所有其他节点，此项必填
	 * 
	 * 数据示例：
	 * */
	...
	/**
	 * 名称：源端VLAN 最大长度:8 是否必填：否 取值范围及说明a侧端口的VLANID 在隧道为Overlay模式下填写 数据示例：
	 * */
	String aUVID = "";
	/**
	 * 名称：宿端rmUID 最大长度: 是否必填：否 取值范围及说明宿端的唯一标识，可能是网元TP点rmUID也可能是MPLS环rmUID
	 * 
	 * 1、当宿端是网元中的TP点，此项填写TP点rmUID；当宿端是MPLS环时，此项填写MPLS环rmUID
	 * 2、对于路由的最后1个节点，此项不填；所有其他节点，此项必填 数据示例：
	 * */
	String zEndTprmUID = "";
	/**
	 * 名称：宿端入标签 最大长度:20 是否必填：否 取值范围及说明1、对于路由的最后1个节点，此项不填 2、所有其他网元节点：
	 * （1）标签交换方向都是双向时，此项必填 （2）标签交换方向是单向时，此项不填 3、当宿端是MPLS保护环时，此项不填 数据示例：26
	 * */
	String zEndInLabel = "";
	/**
	 * 名称：宿端出标签 最大长度:20 是否必填：否 取值范围及说明1、对于路由的最后1个网元节点，此项不填 2、所有其他网元节点，此项必填
	 * 3、当宿端是MPLS保护环时，此项不填 数据示例：32
	 * */
	String zEndRevOutLabel = "";
	/**
	 * 名称：所属网元rmUID 最大长度: 是否必填：是 取值范围及说明1、当源端或宿端是网元上TP点时，填写其所属的网元rmUID 数据示例：
	 * */
	String nermUID;
	/**
	 * 名称：宿端VLAN 最大长度:8 是否必填：否 取值范围及说明z侧端口的VLANID 在隧道为Overlay模式下填写 数据示例：
	 * */
	String zUVID = "";
