...
public class CmOMC extends BaseVO {
	...
	/**
	 * 名称：OMC唯一标识 最大长度: 是否必填：是 取值范围及说明：OMC唯一标识 数据示例：1302HWCS1
	 * */
	String rmUID;
	
	/**
	 * 名称：本地名称 最大长度:255 是否必填是 取值范围及说明 OMC名称 数据示例:MAS-U2000-1-P
	 */
	String nativeName;

	/**
	 * 名称：IP地址 最大长度:40 是否必填：是 取值范围及说明：安装OMC的服务器IP地址 数据示例：10.78.33.22
	 * */
	String commuAddress;

	/**
	 * 名称：软件版本 最大长度:255 是否必填：是 取值范围及说明：网管版本号 数据示例：V1R2C01
	 * */
	String omcVersion;

	/**
	 * 名称：北向接口版本 最大长度:255 是否必填：是 取值范围及说明： 数据示例：2
	 * */
	String interfaceVersion;

	/**
	 * 名称：管理设备类型 最大长度:50 是否必填：是 取值范围及说明：OMC上实际管理的设备类型
	 * 设备类型包括：SDH、WDM、OTN、PTN、PON。 管理多种设备类型时，用逗号分隔，形如：SDH,PTN 数据示例：PTN
	 * */
	String equipmentDomain;

	/**
	 * 名称：设备供应商 最大长度:20 是否必填：是 取值范围及说明：见《OMC系统北向接口数据规范-公共数据分册》 数据示例：HW
	 * */
	String vendor;
	...
	public void setEquipmemtDomain(String equipmemtDomain) {
		if (StringUtils.isEmpty(equipmemtDomain)
				|| equipmemtDomain.length() > 50)
			Asserter.fail(this.getClass(), "equipmemtDomain");
		this.equipmentDomain = equipmemtDomain;
	}
