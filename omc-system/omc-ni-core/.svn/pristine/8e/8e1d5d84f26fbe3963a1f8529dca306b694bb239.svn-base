...
public class CmHolder extends BaseVO {
	...
	String nermUID;
	/**
	 * 名称：本地名称 最大长度:255 是否必填:否 取值范围及说明 :OMC上显示的容器名称 数据示例:OMC=MAS-U2000-1-P
	 */
	String nativeName = "";
	/**
	 * 名称：容器序号 最大长度: 是否必填:否 取值范围及说明 :在所属上层容器中的序号 容器有所属上层容器时必填，十进制 数据示例:
	 */
	String holderNumber = "";
	/**
	 * 名称：容器状态 最大长度: 是否必填:是 取值范围及说明 :枚举值包括： EMPTY：空；
	 ...
	 * UNKNOWN：其它情况 数据示例:INSTALLED_AND_EXPECTED
	 */
	String holderState;
	/**
	 * 名称：容器类型 最大长度: 是否必填:是 取值范围及说明 :枚举值包括： rack：机架； sub_rack：子架； shelf：机框；
	 * sub_shelf：子框； slot：机槽； sub_slot：子槽。 数据示例:slot
	 */
	String holderType;
	/**
	 * 名称：所属上层容器rmUID 最大长度: 是否必填:否 取值范围及说明
	 * :1、当容器类型是子架、机框、子框、机槽、子槽时必填。子架的上层容器只能是机架； 机框的上层容器可以是机架或子架； 子框的上层容器只能是机框；
	 * 机槽的上层容器可以是机框或子框； 子槽的上层容器只能是机槽。 数据示例:
	 */
	...
	public void setNativeName(String nativeName) {
		if (StringUtils.isNotEmpty(nativeName) && nativeName.length() > 255)
