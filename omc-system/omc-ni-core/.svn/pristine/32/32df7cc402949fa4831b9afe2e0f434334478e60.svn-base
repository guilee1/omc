...
public class CmPWTunnel extends BaseVO {
	...
	/**
	 * 名称：承载隧道rmUID 最大长度: 是否必填：是 取值范围及说明： 数据示例：
	 * */
	String tunnelrmUID;

	/**
	 * 名称：归属路由组编号 最大长度: 是否必填：是 取值范围及说明：考虑单向隧道承载双向伪线的情况，标识路由节点归属的路由组。 取值只能为1或2。
	 * 1、当伪线和承载隧道都是双向或单向时，此项固定填数字1 2、当伪线是双向，而承载隧道是单向时，需要按方向将所有承载隧道顺序划分到2个路由组中，
	...
	String routingGroup;
	/**
	 * 名称：路由组中的序号 最大长度: 是否必填：是 取值范围及说明：隧道在归属路由组中的序号。 从1开始，十进制方式 数据示例：1
	 * */
	String number;
