...
public class CmPort extends BaseVO {
	...
	/**
	 * 名称：所属网元rmUID 最大长度: 是否必填：是 取值范围及说明： 数据示例：
	 * */
	String nermUID;
	/**
	 * 名称：所属板卡rmUID 最大长度: 是否必填：否 取值范围及说明：1、当端口归属在板卡下时,此项必填；当端口归属在网元下时，无需填写
	 * 数据示例：
	 * */
	String cardrmUID = "";
	/**
	 * 名称：板卡所属容器rmUID 最大长度: 是否必填：否 取值范围及说明：1、当端口归属在板卡下时,此项必填；当端口归属在网元下时，无需填写
	 * 数据示例：
	 * */
	String holderrmUID = "";
	/**
	 * 名称：端口序号 最大长度: 是否必填：是 取值范围及说明：1、当端口归属在板卡下时,相同“端口分类|端口类型|端口子类型”的端口在板卡下唯一；
	 * 2、当端口归属在网元下时，相同“端口分类|端口类型|端口子类型”的端口在网元下唯一。 3、十进制 数据示例：1
	 * */
	String portNo;
	/**
	 * 名称：本地名称 最大长度:255 是否必填：否 取值范围及说明：OMC上显示的端口名称 数据示例：PORT1
	 * */
	String nativeName = "";
	/**
	 * 名称：端口分类 最大长度: 是否必填：是 取值范围及说明：描述端口是物理口还是逻辑口 枚举值包括： ptp：物理端口； ftp：逻辑端口
	 * 数据示例：PTP
	 * */
	String physicalOrLogical;
	/**
	 * 名称：端口类型 最大长度: 是否必填：是 取值范围及说明：枚举值包括：ETH、TDM、LAG、L2VE、L3VE、other。
	 * 当“端口分类”是ptp时，该项取值只能为ETH、TDM、other；
	 * 当“端口分类”是ftp时，该项取值只能为LAG、L2VE、L3VE、other。 数据示例：ETH
	 */
	String portType;
	/**
	 * 名称：端口子类型 最大长度:19 是否必填：是 取值范围及说明：当某个类型端口下不再区分子类型时，该项填值与“端口类型”属性的填值一致
	 * 数据示例：OptiX PTN 3900
	 */
	String portSubType;
	/**
	 * 名称：端口光电属性 最大长度: 是否必填：是 取值范围及说明：枚举值包括：
	 * optical：光口；electrical：电口；invalid：无效。
	 * 当“端口分类”是ptp时，必须指定是optical或electrical； 当“端口分类”是ftp时，必须指定为invalid。
	 * 数据示例：electrical
	 */
	String signalType;
	/**
	 * 名称：端口速率 最大长度: 是否必填：是 取值范围及说明：见字典表 数据示例：GE
	 */
	String portRate;
	/**
	 * 名称：方向 最大长度: 是否必填：是 取值范围及说明：枚举值包括： D_BIDIRECTIONAL：双向； D_SOURCE：源；
	 * D_SINK：宿； DIR_NA：未知 数据示例：D_BIDIRECTIONAL
	 */
	String direction;
	/**
	 * 名称：端口角色 最大长度: 是否必填：是 取值范围及说明：枚举值包括： Master：主用； Backup：备用； NA：无主备端口
	 * 数据示例：Master
	 */
	String role;
	/**
	 * 名称：端口IP 最大长度:40 是否必填：否 取值范围及说明：当三层端口上配置时此项必填： 数据示例：10.77.66.11
	 */
	String IPAddress = "";
	/**
	 * 名称：子网掩码 最大长度:40 是否必填：否 取值范围及说明：当三层端口上配置时此项必填 数据示例：
	 */
	String IPMask = "";
	/**
	 * 名称：是否overlay端口 最大长度: 是否必填：是 取值范围及说明：标识端口是否是overlay方式 枚举值包括：
	 * true：overlay方式；false：非overlay方式 数据示例：true
	 */
	String isOverlay;
