...
public class ResFileUtil {
	public static String SPECIALUID = "this is a fake uid for just generate file header";
	...
	public static DataFile convertToDataFile(final List<? extends BaseVO> objList){
		...
		dataFile.getObjects().add(objects);
		Class<?> clazz = objList.get(0).getClass();
		objects.setObjectType(objList.get(0).getResType());
    ...
    static FieldValue buildFieldValue(final List<? extends BaseVO> objList) {
    	...
    	if(objList.isEmpty())
    		return fieldValue;
    	BaseVO specialOne = objList.get(0);
    	if(SPECIALUID.equals(specialOne.getFileRmUID()))
    		return fieldValue;
    	List<String> fieldNames = ReflectionUtil.getFieldNameList(specialOne.getClass());
    ...
    public static DataFile.Objects.FieldValue.Object buildFieldObject(BaseVO resObj){
    	List<String> fieldNames = ReflectionUtil.getFieldNameList(resObj.getClass());
	...
	static DataFile.Objects.FieldValue.Object buildFieldObject(List<String> fieldNames, BaseVO resObj) {
		...
		for(int i=0;i<fieldNames.size();++i){
			...
			v.setValue(ReflectionUtil.getFieldVal(resObj,fieldNames.get(i)));
			object.getV().add(v);
	...
	static FieldName buildFieldName(Class<?> clazz) {
    	...
    	List<String> fieldNames = ReflectionUtil.getFieldNameList(clazz);
    	for(int i=0;i<fieldNames.size();++i){
    		N n = new N();
	...
	static FileHeader buildHeader() {
    	...
    	header.setCmVersion(Constants.PM_VERSION);
    	header.setElementType(Constants.ELEMENT_TYPE);
    	header.setVendorName(Constants.VENDOR_NAME);
