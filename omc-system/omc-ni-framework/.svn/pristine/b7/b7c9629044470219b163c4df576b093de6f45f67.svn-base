...
public class ResInfImpl implements IResInf {
	...
	@Override
	public List<CmPort> getEthPort() {
		List<CmPort> result = new ArrayList<>();
		List<IResInf> resInfs = locator.getInstances(IResInf.class);
		for(IResInf resInf : resInfs){
			List<CmPort> oneList = resInf.getEthPort();
			if(oneList != null && !oneList.isEmpty()){
				result.addAll(oneList);
			}
		}
	...
	@Override
	public List<CmEProtectGroup> getCmEProtectGroup() {
		List<CmEProtectGroup> result = new ArrayList<>();
		List<IResInf> resInfs = locator.getInstances(IResInf.class);
		for(IResInf resInf : resInfs){
			List<CmEProtectGroup> oneList = resInf.getCmEProtectGroup();
			if(oneList != null && !oneList.isEmpty()){
				result.addAll(oneList);
			}
		}
	...
	@Override
	public List<CmEProtectGroupUnit> getCmEProtectGroupUnit() {
		List<CmEProtectGroupUnit> result = new ArrayList<>();
		List<IResInf> resInfs = locator.getInstances(IResInf.class);
		for(IResInf resInf : resInfs){
			List<CmEProtectGroupUnit> oneList = resInf.getCmEProtectGroupUnit();
			if(oneList != null && !oneList.isEmpty()){
				result.addAll(oneList);
			}
		}
	...
	@Override
	public List<CmProtectGroup> getCmProtectGroup() {
		List<CmProtectGroup> result = new ArrayList<>();
		List<IResInf> resInfs = locator.getInstances(IResInf.class);
		for(IResInf resInf : resInfs){
			List<CmProtectGroup> oneList = resInf.getCmProtectGroup();
			if(oneList != null && !oneList.isEmpty()){
				result.addAll(oneList);
			}
		}
	...
	@Override
	public List<CmProtectGroupUnit> getCmProtectGroupUnit() {
		List<CmProtectGroupUnit> result = new ArrayList<>();
		List<IResInf> resInfs = locator.getInstances(IResInf.class);
		for(IResInf resInf : resInfs){
			List<CmProtectGroupUnit> oneList = resInf.getCmProtectGroupUnit();
			if(oneList != null && !oneList.isEmpty()){
				result.addAll(oneList);
			}
		}
	...
	@Override
	public List<CmPort> getPdhPort() {
		List<CmPort> result = new ArrayList<>();
		List<IResInf> resInfs = locator.getInstances(IResInf.class);
		for(IResInf resInf : resInfs){
			List<CmPort> oneList = resInf.getPdhPort();
			if(oneList != null && !oneList.isEmpty()){
				result.addAll(oneList);
			}
		}
	...
	@Override
	public List<CmPort> getSubPort() {
		List<CmPort> result = new ArrayList<>();
		List<IResInf> resInfs = locator.getInstances(IResInf.class);
		for(IResInf resInf : resInfs){
			List<CmPort> oneList = resInf.getSubPort();
			if(oneList != null && !oneList.isEmpty()){
				result.addAll(oneList);
			}
		}
	...
	@Override
	public List<CmPort> getOtherPort() {
		List<CmPort> result = new ArrayList<>();
		List<IResInf> resInfs = locator.getInstances(IResInf.class);
		for(IResInf resInf : resInfs){
			List<CmPort> oneList = resInf.getOtherPort();
			if(oneList != null && !oneList.isEmpty()){
				result.addAll(oneList);
			}
		}
